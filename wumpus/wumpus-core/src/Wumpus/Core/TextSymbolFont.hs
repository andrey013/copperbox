...
type SymbolCharName = String
type SymbolOctCode  = Int

-- | Latin1 TextEncoder instance.
symbolEncoder :: TextEncoder
symbolEncoder = TextEncoder {
...
nameToCode = Map.fromList symbolAll

codeToName :: Map.Map Int String
codeToName = foldr fn Map.empty symbolAll where
  fn (s,i) a = Map.insert i s a 

-- | A lookup list of Latin 1 names to their octal code.
symbolAll :: [(SymbolCharName, SymbolOctCode)]
symbolAll = [ ("Alpha",                 0o101)
            , ("Beta",                  0o102)
            , ("Chi",                   0o103)
            , ("Delta",                 0o104)
            , ("Epsilon",               0o105)
