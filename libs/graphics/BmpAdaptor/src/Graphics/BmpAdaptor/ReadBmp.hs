...
--------------------------------------------------------------------------------
-- |
-- Module      :  Graphics.BmpAdaptor.ReadBmp
...
import Graphics.BmpAdaptor.Utils ( cstyle2Darray, sectionSizes)
...
getWords :: Int -> Parser [Word8]
getWords n = do
    bs <- gets remaining
    if (n <= BS.length bs) 
        then let (ls,rs) = n `BS.splitAt` bs in do
            i <- gets pos
            modify $ \s -> s { remaining=rs, pos=i+n } 
            return $ BS.unpack ls
        else reportFail "Unexpected eof"
