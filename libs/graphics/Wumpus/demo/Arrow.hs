...
demo1 = writePicture "arrow1.ps" drawing1 where
  ...
  arr1      = picPath stroke $ arrow (P2 0 0) (P2 40 0) 

  arrheads1 = displace 50 0 $ withRgbColour dodgerBlue1 $ cat
                [ arrowhead1 (P2 0 0) 
                             (P2 10 50) 
                             (picPolygon  stroke `oo` arrowheadTriangle 10 (pi/10))
                , arrowhead1 (P2 10 0) 
                             (P2 20 50) 
  ...
  picLines  = picPath stroke . segmentPath 

  cm1       = displace 100 0 $ picPath stroke $ segmentPath $ arrowCenterMarker $ 
                LS (P2 30 0) (P2 40 50)
  ...
  subs1     = let (a,b) = subdividet (0.9::Double) $ testCurve (pi/2) in
              displace 0 230 $ 
                       (withRgbColour brown1 $ picPath stroke $ bezierPath a)
                  <..> (withRgbColour chartreuse1 $ picPath stroke $ bezierPath b)
...
arrowhead1 start_pt end_pt arrHead = 
    (picPath stroke $ straightLine arrline) <..> (arrHead theta end_pt)
  where
    theta    :: Radian
    arrline  = LS start_pt end_pt
...
splitCurve crv =  
         (withRgbColour darkGoldenrod1 $ picPath stroke $ bezierPath a)
    <..> (withRgbColour cyan4 $ picPath stroke $ bezierPath b)
  where
