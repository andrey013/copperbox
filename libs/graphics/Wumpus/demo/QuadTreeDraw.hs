...
drawQuadTree tree = drawing1 where
  drawing1 = displace 80 300 (picLines (treeLines tree) <..> points)
...
sqlines (P2 x0 y0, P2 x1 y1) = [t,r,b,l]  -- clockwise
  where
    t = LS (P2 x0 y1) (P2 x1 y1)
    r = LS (P2 x1 y1) (P2 x1 y0)
    b = LS (P2 x1 y0) (P2 x0 y0)
    l = LS (P2 x0 y0) (P2 x0 y1)

midlines :: (DPoint2,DPoint2) -> [DLineSegment2]
midlines (p1@(P2 x0 y0), p2@(P2 x1 y1)) = [v,h]  -- clockwise
  where
    P2 x' y' = midpoint p1 p2
    v        = LS (P2 x' y0) (P2 x' y1)
    h        = LS (P2 x0 y') (P2 x1 y')
