...
parallelPhrases :: Duration -> [LyPhrase] -> [OverlayBar]
parallelPhrases _       []     = []
parallelPhrases bar_len (x:xs) = snd $ foldl' (parallel2 bar_len) (1,parallel1 x) xs

type Depth = Int

parallel2  :: Duration -> (Int,[OverlayBar]) -> LyPhrase -> (Int,[OverlayBar])
parallel2 bar_len (depth,bs1) (LyPhrase bs2) = (depth+1, step bs1 bs2) where
  step (x:xs) (y:ys) = parallelLy x (getLyBar y) : step xs ys
  step (x:xs) []     = parallelLy x (spacer $ Just bar_len) : step xs [] 
  step []     (y:ys) = parallelLy (blank depth bar_len) (getLyBar y) : step [] ys 
  step []     []     = []
...
parallel1 :: LyPhrase -> [OverlayBar]
parallel1 = map (OverlayBar . getLyBar) . getLyPhrase
