...
> vol v (Primitive (Note d p)) = Primitive (Note d (p, [Volume v]))
> vol v (Primitive (Rest d)) = Primitive (Rest d)
...
> v1b = lmap va      [cs 5 tn, d 5 (qn-tn), cs 5 en, b 4 en]
> 
> v2  = v2a :+: v2b :+: v2c :+: v2d :+: v2e :+: v2f
> v2a = lmap va [cs 5 (dhn+dhn), d 5 dhn, 
>                f 5 hn, gs 5 qn, fs 5 (hn+en), g 5 en]
> v2b = lmap (fd en) [fs 5, e 5, cs 5, as 4] :+: va (a 4 dqn) :+:
>       lmap (fd en) [as 4, cs 5, fs 5, e 5, fs 5, g 5, as 5]
> v2c = va (line [cs 6 (hn+en), d 6 en, cs 6 en, e 5 en] :+: enr :+: 
>           line [as 5 en, a 5 en, g 5 en, d 5 qn, c 5 en, cs 5 en])
> v2d = lmap (fd en) [fs 5, cs 5, e 5, cs 5, a 4, as 4, d 5, e 5, fs 5] :+:
>       lmap va [fs 5 tn, e 5 (qn-tn), d 5 en, e 5 tn, d 5 (qn-tn),
>                cs 5 en, d 5 tn, cs 5 (qn-tn), b 4 (en+hn)]
> v2e = lmap va [cs 5 en, b 4 en, fs 5 en, a 5 en, b 5 (hn+qn), a 5 en,
>                fs 5 en, e 5 qn, d 5 en, fs 5 en, e 5 hn, d 5 hn, fs 5 qn]
> v2f = va $ Modify (Tempo (3/2)) $
>         (line [cs 5 en, d 5 en, cs 5 en]) :+: (b 4 (3*dhn+hn))
> 
> mainVoice = times 3 v1 :+: v2
> 
> -- Putting it all together:
> childSong6 = Modify (Instrument AcousticGrandPiano) 
>                (Modify (Tempo 3) 
>                  (Modify (Phrase [Dyn (StdLoudness SF)]) 
>                    bassLine :=: mainVoice))
