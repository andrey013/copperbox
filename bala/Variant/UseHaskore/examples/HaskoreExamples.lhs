...
> t0 = play (Modify (Instrument AcousticGrandPiano) childSong6)
...
> rep f g 0 m = Primitive (Rest 0)
> rep f g n m = m :=: g (rep f g (n-1) (f m))

An example using "rep" three times, recursively, to create a "cascade"
of sounds.

> run       = rep (Modify (Transpose 5)) (delay tn) 8 (c 4 tn)
> cascade   = rep (Modify (Transpose 4)) (delay en) 8 run
...
> run'      = rep (delay tn) (Modify (Transpose 5)) 4 (c 4 tn)
> cascade'  = rep (delay en) (Modify (Transpose 4)) 6 run'
...
> t10s   = play (rep (delay durss) (Modify (Transpose 4)) 2 ss)
...
>                   (line (map (\p-> Primitive $ Note sn (pitch p)) [a..b]))
...
> t12a = play (fmap (\p -> (p,[])) cms /=: childSong6)

-----------------------------------------------------------------------------

Tests of the trill functions.

> t13note = Primitive (Note qn (C,5))
