...
> root       p dur = Primitive $ Note dur p
> minThird   p dur = Primitive $ Note dur (trans 3 p)
> majThird   p dur = Primitive $ Note dur (trans 4 p)
> fifth      p dur = Primitive $ Note dur (trans 7 p)
> majSixth   p dur = Primitive $ Note dur (trans 9 p)
> minSeventh p dur = Primitive $ Note dur (trans 10 p)
> octave     p dur = Primitive $ Note dur (trans 12 p)
> oMinThird  p dur = Primitive $ Note dur (trans 15 p)
> oFifth     p dur = Primitive $ Note dur (trans 19 p)
...
> diddle p = snr :+: Primitive (Note sn p) 
>            :+: Primitive (Note sn (prevNR p)) :+: Primitive (Note sn p)
...
> endEl n = Primitive (Note sn n)          :+: Primitive (Note sn (back2NR n))
>       :+: Primitive (Note sn (prevNR n)) :+: Primitive (Note sn n)

> endRun = line $ map endEl $ take 10 $ iterate nextNR (D,5)

> ending = all3Insts $
>       Primitive (Note qn (D,5))
