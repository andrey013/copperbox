...
instance PPrint Pitch where
  pprint (Pitch p oa i) = pprint p <> (mbDoc pprint oa) <> int i

instance PPrint PitchLetter where
  pprint  = text . show


instance PPrint Accidental where
  pprint DoubleFlat   = text "bb"
  pprint Flat         = char 'b'
  pprint Nat          = empty
  pprint Sharp        = char '#'
  pprint DoubleSharp  = text "##"

instance PPrint PitchLabel where
  pprint (PitchLabel p oa) = pprint p <> (maybe empty pprint oa)
