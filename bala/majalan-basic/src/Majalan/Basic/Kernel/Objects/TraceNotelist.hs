...
class TraceM (m :: * -> *) where
  trace     :: (DUnit (m ()) ~ u, UCtx m  ~ ctx) => HPrim ctx u -> m ()




newtype Notelist ctx u a   = Notelist { 
          getNotelist :: Context ctx -> (a, HPrim ctx u) }

newtype NotelistT ctx u m a = NotelistT { 
          getNotelistT :: Context ctx -> m (a, HPrim ctx u) }
...
runNotelist :: Context ctx -> Notelist ctx u a -> (a, HPrim ctx u)
...
--
execNotelist :: Context ctx -> Notelist ctx u a -> HPrim ctx u
...
runNotelistT :: Monad m 
             => Context ctx -> NotelistT ctx u m a -> m (a, HPrim ctx u) 
runNotelistT ctx ma = getNotelistT ma ctx

execNotelistT :: Monad m 
              => Context ctx -> NotelistT ctx u m a -> m (HPrim ctx u)
