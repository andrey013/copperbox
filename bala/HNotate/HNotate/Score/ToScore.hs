...
processSystem :: (Event evt) => System evt -> ProcessM ScScore
processSystem p@(System xs) = ScScore <$> foldM fn mempty (zip xs [1..])
  where
...
toScore1 :: (Event evt)
         ...
         -> Notate_Sc_Env
         -> ScScore
toScore1 tree env = toScore (System [tree]) env
