...
type VoiceOverlayB = (Int, Duration, Seq ScoreGlyph)
...
eventsToNoteList :: Duration -> Duration -> EventList -> ScoreNoteList
...
eventsToNoteList_debug bar_len partial_start = 
  ...
  where 
    ...
    o4 = genWriteStep "Finally the note list..."        ppScoreNoteList 
...
reduceTreeStep bar_len (bc,d,se) = vals $ F.foldl fn (bc,d,empty,[]) se
  where
    ...
    spacerPrefix d (EventSeq se) | d > 0     = Evt (SgSpacer d) <| se
                                 | otherwise = se

      

type RawBar = (Int,Seq ScoreGlyph)

type PVobSt = (Int,Duration,Seq ScoreGlyph)
...
partitionVoB bar_len (bar_num, partial, se) = 
  ...
  where
    ...
    plusop :: (Int,Duration) -> ScoreGlyph -> (Int,Duration)
    ...
    together :: ((Int,Duration), Seq ScoreGlyph, Maybe ScoreGlyph, Seq ScoreGlyph)
                  -> (Int, Duration, Seq ScoreGlyph, Seq ScoreGlyph)
    ...
    together ((bc,d), sl, Just e,  sr) = let ed = glyphDuration e in 
        case d + ed == bar_len of
          ...
          False -> let leftd  = bar_len - d
                       rightd = ed - leftd
                       lg     = onDuration (const leftd) e
                       rg     = onDuration (const rightd) e
                   -- new duration is 0 as the 'right' glyph
                   -- is enqueued and will be consumed later
                   in (bc, 0, sl |> lg |> SgTie, rg <| sr)
...
-- Change the raw bars to ScoreMeasure and queue them on 
-- bar number. 
rawToQueue :: [Seq RawBar] -> OnsetQueue ScoreMeasure
rawToQueue = buildQueue fst snd . rebuild
  where 
    rebuild :: [Seq RawBar] -> Seq (Int,ScoreMeasure)
    rebuild = foldr (><) empty . map (build1 empty)
    
    build1 :: Seq (Int,ScoreMeasure) -> Seq RawBar -> Seq (Int,ScoreMeasure)
    build1 = F.foldr (\(i,se) sa -> if null se then sa
                                               else sa |> (i, ScMeasure se))
 
-- Finally linearize the queue - aggregating simultaneous
-- bars into voice overlays (PolyBlock's) 
collapseQueue :: OnsetQueue ScoreMeasure -> ScoreNoteList 
collapseQueue = ScNoteList . foldlOnsetQueue fn empty
  where    
    fn acc (i, [x]) = acc |> (ScSingleBlock i x)
    fn acc (i, xs)  = acc |> (ScPolyBlock i (fromList xs))
...
ppOnsetQueue :: OnsetQueue ScoreMeasure -> Doc
ppOnsetQueue = foldlOnsetQueue fn PP.empty
  where fn d (i,xs) = d <$> int i <+> text ":+" <+> list (map pretty xs)
  
ppScoreNoteList :: ScoreNoteList -> Doc
ppScoreNoteList = pretty
