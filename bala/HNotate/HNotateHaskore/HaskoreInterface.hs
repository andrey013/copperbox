...
data NrEvent = EvtNote Pitch Duration
             | EvtRest Duration
  deriving (Eq,Show)

instance Event NrEvent where
  eventvalues (EvtNote p d) = (Just $ renderPitch p, renderDuration d)
  eventvalues (EvtRest d)   = (Nothing, renderDuration d)


psystem :: H.Performance -> System NrEvent
...
instEL :: (H.InstrumentName,H.Performance) 
       -> EventList NrEvent
  ...
  where
    fn (onset,tree) e   = let (e_onset, e_dur, f) = evt e in 
        if e_onset == onset
          ...
          else let r = mkRest (e_onset - onset)
               in (e_onset + e_dur, tree # event r # f)

groupChords :: [H.Event] -> [[H.Event]]
groupChords = groupBy (\a b -> H.eTime a == H.eTime b)

    
evt :: [H.Event] -> (H.Time, H.DurT, EventList NrEvent -> EventList NrEvent)
evt [e]     = event1 e
evt (e:es)  = let (e_onset, e_dur,_)  = event1 e
                  chord_notes         = sortBy ascend $ map mkEvtNote (e:es) 
              in (e_onset, e_dur, par chord_notes)
  where
    ascend (EvtNote p _) (EvtNote p' _) = p `compare` p'
    ascend  _            _              = EQ              

event1 e@(H.Event {H.eTime=onset, H.eDur=drn}) = 
    (onset,drn, event (mkEvtNote e))


mkEvtNote :: H.Event -> NrEvent
mkEvtNote evt = case eventvalues evt of
  (Just p, d)  -> EvtNote p d
  (Nothing, d) -> EvtRest d     
      
mkRest :: Rational -> NrEvent
mkRest r = EvtRest $ convert r   
...
instance Event H.Event where
  eventvalues (H.Event {H.ePitch = p, H.eDur = d})
      = (Just $ convert p , convert d)

instance Event (H.Primitive H.Pitch) where
  eventvalues (H.Note d a) = (Just $ convert a, convert d)
  eventvalues (H.Rest d)   = (Nothing, convert d)


instance Event (H.PitchClass, H.Octave) where
  eventvalues a = (Just $ convert a, quarter)
