...
readMidi name = do 
    ...
    sz <- hFileSize h
    cs <- hGetContents h
    ans <- parseIO midiFile name cs
    -- The `putStrLn ` line seems necessary otherwise we get a premature 
    -- close of the file handle
    putStrLn $ "Read " ++ show (length cs) ++ " bytes."
    hClose h
    case ans of 
      Left err -> error $ show err
      Right mf -> return mf

parseIO :: Parser a -> FilePath -> String -> IO (Either ParseError a)
parseIO p name cs = let ans = parse p name cs in return ans
...
getWord32be = w32be <$> getWord16be <*> getWord16be
...
w32be :: Word16 -> Word16 -> Word32
w32be a b = let a' = (fromIntegral a) `shiftL` 16
                b' = fromIntegral b
            in a' + b'         
