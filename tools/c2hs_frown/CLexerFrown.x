...
type Alex token = State (AlexState token) token

data Lexeme = L AlexPosn Terminal String
  deriving (Show)


instance Token Lexeme where emptyTok = (L undefined TK_EOF "")

mkL :: (String -> Terminal) -> AlexInput -> Int -> Alex Lexeme
mkL fn (p,_,str) len = return (L p (fn chars) chars)
  where chars = take len str

mkKW :: Terminal -> AlexInput -> Int -> Alex Lexeme
mkKW cnstr (p,_,str) len = return (L p cnstr chars)
  where chars = take len str


mkSpecial ::  AlexInput -> Int -> Alex Lexeme
mkSpecial = mkL makeTerminal

mkKeyword ::  AlexInput -> Int -> Alex Lexeme
mkKeyword = mkL makeTerminal
    
mkIdentifier :: AlexInput -> Int -> Alex Lexeme
mkIdentifier = mkL TK_Identifier 


mkStringLiteral = mkL makeStringLit

mkCInteger :: AlexInput -> Int -> Alex Lexeme
mkCInteger = mkL TK_Integer 

mkCFloat :: AlexInput -> Int -> Alex Lexeme
mkCFloat = mkL TK_Float 

mkCChar :: AlexInput -> Int -> Alex Lexeme
mkCChar = mkL TK_Char 

alexEOF :: Alex Lexeme
alexEOF = return (L undefined TK_EOF  "")
